...
| **LPIPS** ↓ | **0.312** (raw) / 0.130 (corr) | 0.30 | 0.30 |
...
| 指标 | 值 | 说明 |
|------|-----|------|
| **PSNR** ↑ | **8.12 dB** (直接评估, 未做 tone-mapping 校正) | GT/EST 分辨率不同 (1024² vs 720p), resize 到 320² |
| **SSIM** ↑ | **0.1097** | 真实事件噪声大, 结构重建更难 |
| **LPIPS** ↓ | N/A | GT/EST分辨率不匹配 |

> mocap-desk 的 GT 和渲染图来自**不同相机** (RGB cam vs Event cam), 亮度/分辨率都不匹配。comppsnr.py 的 tone-mapping 校正要求同尺寸, 无法直接应用。此 PSNR 为原始未校正值。
